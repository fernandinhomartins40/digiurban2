...
// Mark alert as read
router.patch('/alerts/:alertId/read', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    await pool.query(`
      UPDATE alert_delivery_stats 
      SET read_count = (
        SELECT COUNT(*) FROM alert_recipients 
        WHERE alert_id = $1 AND is_read = true
      )
      WHERE alert_id = $1
    `, [alertId]);
