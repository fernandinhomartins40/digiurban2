-- =====================================================
-- 15. CORREÇÃO TABELA PROTOCOLOS
-- =====================================================

-- Adicionar colunas faltantes na tabela protocolos existente
ALTER TABLE protocolos ADD COLUMN IF NOT EXISTS responsavel_id UUID REFERENCES user_profiles(id);
...
ALTER TABLE protocolos ADD COLUMN IF NOT EXISTS avaliacao INTEGER CHECK (avaliacao >= 1 AND avaliacao <= 5);
...
ALTER TABLE protocolos ADD COLUMN IF NOT EXISTS prioridade VARCHAR(10) DEFAULT 'normal' CHECK (prioridade IN ('baixa', 'normal', 'alta', 'urgente'));
...
-- Criar as outras tabelas necessárias
...
DECLARE
  prazo_dias INTEGER;
BEGIN
  -- Buscar o prazo do serviço ou usar padrão
  SELECT prazo_resposta_dias INTO prazo_dias
  FROM servicos_municipais
  WHERE id = NEW.servico_id;
  
  -- Calcular a data do prazo
  NEW.data_prazo := NEW.created_at + (COALESCE(prazo_dias, 15) || ' days')::INTERVAL;
...
DROP POLICY IF EXISTS "Sistema pode criar histórico" ON protocolos_historico;
CREATE POLICY "Sistema pode criar histórico" ON protocolos_historico 
