...
export const protocolService = {
  ...
  async getServicosByCategoria(categoriaId: string): Promise<ServicoMunicipal[]> {
    const { data, error } = await supabase
  ...
  async getServicosPublicos(): Promise<ServicoMunicipal[]> {
    const { data, error } = await supabase
      .from('servicos_municipais')
      .select(`
        *,
  ...
  async getServicoById(id: string): Promise<ServicoMunicipal | null> {
    const { data, error } = await supabase
      .from('servicos_municipais')
      .select(`
        *,
  ...
  async getProtocolosUsuario(status?: string): Promise<Protocolo[]> {
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) throw new Error('Usuário não autenticado')
  ...
  async getProtocoloById(id: string): Promise<Protocolo | null> {
    const { data, error } = await supabase
      .from('protocolos')
      .select(`
        *,
  ...
  async getHistoricoProtocolo(protocoloId: string): Promise<ProtocoloHistorico[]> {
    const { data, error } = await supabase
